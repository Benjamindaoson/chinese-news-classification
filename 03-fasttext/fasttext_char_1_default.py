...
model = fasttext.train_supervised(
    ...
    dim=10, # 词向量的维度
    wordNgrams=3,   # N-gram特征的N
    epoch=10,    # 训练轮数
    lr=0.2, # 学习率
)

# 3.模型保存
# ft_model.bin -> ft_model_char_1.bin
model_path = config.ft_model_path.replace(".bin", "_char_1.bin")
save_path, final_path = fasttext_write_path(model_path)
model.save_model(save_path)
copy_back(save_path, final_path)
print(f'模型保存到: {config.ft_model_path.replace(".bin", "_char_1.bin")}')
# 4.模型评估
