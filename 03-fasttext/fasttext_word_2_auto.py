...
model = fasttext.train_supervised(
    ...
    autotuneValidationFile = fasttext_read_path(config.word_dev_path),  # 自动调参的验证集文件路径
...
model_path = config.ft_model_path.replace(".bin", "_word_2.bin")
save_path, final_path = fasttext_write_path(model_path)
model.save_model(save_path)
copy_back(save_path, final_path)
print(f'模型保存到: {config.ft_model_path.replace(".bin", "_word_2.bin")}')
# 4.模型评估
