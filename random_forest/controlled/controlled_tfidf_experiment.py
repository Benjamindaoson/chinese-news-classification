...
REPORT_DIR = ROOT / "reports" / "random_forest" / "controlled"
ARTIFACT_DIR = ROOT / "artifacts" / "random_forest" / "controlled"
...
def read_stopwords() -> list[str]:
    return [
        line.strip()
        for line in (ROOT / "data" / "stopwords.txt").read_text(encoding="utf-8-sig").splitlines()
        if line.strip()
    ]
