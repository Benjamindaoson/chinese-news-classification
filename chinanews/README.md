...
`prepare_dataset.py` 默认兼容无表头 CSV：第一列为类别编号，其余文本列会拼接为分类输入。如果你的数据格式不同，可通过命令行指定分隔符。

## 数据划分原则

官方数据只有训练集与测试集。本项目从官方训练数据中使用**确定性哈希划分**生成约 5% 的验证集，并在百万级均衡数据下检查各类别分布是否保持基本一致；官方测试集始终仅用于最终评测：
...
这样可以避免把测试集反复用于超参数选择造成评测泄漏，同时保证同一输入在重复运行时始终落入相同数据划分。
