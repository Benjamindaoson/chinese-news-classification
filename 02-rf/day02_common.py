...
DATA = ROOT / "01-data"
REPORTS = ROOT / "reports" / "day02"
ARTIFACTS = ROOT / "artifacts" / "day02"
...
- `02-rf/data_process.py` reads `train.txt`, `test.txt`, and `dev.txt`, applies `jieba.lcut(text)[:20]` into `words`, computes `seq_len` as full jieba token count, and writes `process_*.csv`.
...
- `02-rf/rf_train.py` reads `process_train.csv` and truncates to the first 20000 rows.
...
- Original files save to `01-data/model/rf_model.pkl` and `01-data/model/tfidf_model.pkl`; this reproduction writes only to `artifacts/day02/baseline`.
...
v2 uses `01-data/v2/train_v2.txt`, `dev_v2.txt`, and `test_v2.txt`. Generated process CSV files are under `01-data/v2/process_*_v2.csv`.
...
See `reports/day02/v2/error_samples.csv`. RandomForest on short Chinese titles mainly fails around short titles, mixed-topic titles, fuzzy class boundaries, proper nouns, numbers/abbreviations, segmentation limitations, stopword effects, and label noise. It should not be described as understanding news semantics.

## Saved Model Location

- `artifacts/day02/v2/tfidf_v2.pkl`
- `artifacts/day02/v2/rf_v2.pkl`

## Reproducible Command

`python 02-rf/run_day02.py --mode v2`
"""
...
"""
        (REPORTS / "DAY02_COMPARISON.md").write_text(content, encoding="utf-8")
...
def main() -> int:
    ...
    run(args.mode)
    print(f"day02 mode {args.mode} finished")
    return 0
