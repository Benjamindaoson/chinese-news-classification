...
需要掌握的：
    ...
    2.启动streamlit程序: streamlit run D:\tmf_project_shanghai_AI_2\08-live_code\random_forest\streamlit_app.py
"""
...
# 2.1 构造网址url
url = f"http://{config.api_host}:{config.api_port}/predict1"
# 2.2 设置 预测按钮 的逻辑
with col1:
    if st.button("bert预测"):
        try:
...
# 2.2 构造网址url
url = f"http://{config.api_host}:{config.api_port}/predict2"
# 2.2 设置 预测按钮 的逻辑
with col2:
    if st.button("bert蒸馏版预测"):
        try:
            # 0.记录开始时间
            start_time = time.time()
            # 1.使用requests发送请求体，调用requests.post来获取模型预测结果
