...
class Config():
    ...
    def __init__(self):
        # 1.初始化根路径
        self.root_path = os.path.join(Path(__file__).parent.parent, "data")
        # self.root_path = r"D:\tmf_project_shenzhen_AI_4\07_live_code\data"
        # 2.初始化数据文件路径
        ...
        # 学生模型保存路径
        self.student_model_path = os.path.join(self.root_path, "model", "student_model.pt")
        ...
        self.epochs = 10  # 训练轮数, BERT微调通常3~5轮
        self.batch_size = 64  # 每批次样本数量
        self.lr = 2e-4  # 学习率, BERT微调通常1e-5~5e-5
        self.max_len = 32  # 最大序列长度
        self.weight_decay = 1e-3  # 权重衰减, AdamW优化器参数
        self.alpha = 0.7  # 软标签蒸馏的权重系数
        self.T = 4  # 软标签蒸馏的温度系数
