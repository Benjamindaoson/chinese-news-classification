...
import time
from bert_train import evaluate
...
class MyStudentModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.student_config = BertConfig(
            vocab_size=BERT_CONFIG.vocab_size,  # 词表大小
            hidden_size=256,    # 特征维度
            num_hidden_layers=2,    # 编码器层层数
            num_attention_heads=8,   # 注意力头数
            intermediate_size=256*4,   # 隐藏层维度
...
# 4.1 训练一个轮次 - 要求手敲
def train_one_epoch(
    train_dataloader, teacher_model, student_model,
    ...
    :param ce_loss_fn: 多分类交叉熵损失函数，用于计算 学生输出student_logtis 和 真实标签 之间的损失
    :param kl_loss_fn: KL散度损失函数，用于计算 学生输出student_logtis 和 教师输出teacher_logtis 之间的损失
    :param optimizer: 优化器
    :return: 学生模型的训练损失、准确率、f1分数
    """
    # 1.设为训练模式
    student_model.train()
    ...
    for batch in tqdm(train_dataloader, desc="Training"):
        ...
        # 教师输出
        with torch.no_grad():
            teacher_logits = teacher_model(input_ids, attention_mask)   # (B,10)
        # 学生输出
        student_logits = student_model(input_ids, attention_mask)   # (B,10)
        # 2.计算损失
        # celoss
        ce_loss = ce_loss_fn(student_logits, labels)
        # kl散度损失，student_logits, teacher_logits
        # softmax(teacher_logits/config.T)
        # log_softmax(student_logits/config.T)
        kl_loss = kl_loss_fn(
            torch.log_softmax(student_logits/config.T,dim=-1),torch.softmax(teacher_logits/config.T,dim=-1))
        # 计算总损失
        ...
        # 获取当前批次的预测结果
        y_preds = torch.argmax(student_logits, dim=-1)  # (B,)
...
def evaluate_student(val_dataloader, teacher_model, student_model, ce_loss_fn, kl_loss_fn):
    """
    验证一个轮次，返回验证损失、准确率、f1分数
    ...
    for batch in tqdm(val_dataloader, desc="Evaluating"):
        ...
        # softmax(teacher_logits/config.T)
        # log_softmax(student_logits/config.T)
        kl_loss = kl_loss_fn(torch.log_softmax(student_logits/config.T,dim=-1),torch.softmax(teacher_logits/config.T,dim=-1))
        # 计算总损失
        loss = config.alpha * config.T**2 * kl_loss + (1-config.alpha) * ce_loss
        # # 3.梯度清零
        # optimizer.zero_grad()
...
def train_student():
    ...
    for epoch in range(config.epochs):
        ...
        train_loss, train_acc, train_f1 = train_one_epoch(
            train_dataloader, teacher_model, student_model, ce_loss_fn, kl_loss_fn, optimizer)
        # 2.评估模型
        val_loss, val_acc, val_f1 = evaluate_student(
            dev_dataloader, teacher_model, student_model, ce_loss_fn, kl_loss_fn)
        ...
        if val_f1 > best_val_f1:
            ...
            print(f"保存模型到{config.student_model_path}, 最优验证F1分数: {val_f1:.4f}")
        # 4.添加训练指标到列表中
...
if __name__ == '__main__':
    ...
    print(student_model)
    # 评估教师模型
    test_loss, test_acc, test_f1 = evaluate(test_dataloader, teacher_model, loss_fn=nn.CrossEntropyLoss())
    print(f"教师模型测试结果: loss: {test_loss:.4f}, acc: {test_acc:.4f}, f1: {test_f1:.4f}")
    # 评估学生模型
    test_loss, test_acc, test_f1 = evaluate(test_dataloader, student_model, loss_fn=nn.CrossEntropyLoss())
    print(f"学生模型测试结果: loss: {test_loss:.4f}, acc: {test_acc:.4f}, f1: {test_f1:.4f}")